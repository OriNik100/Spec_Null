...
* **Multi-Objective Loss Function:** Designed a comprehensive loss function that balances several inherently conflicting objectives, including null energy minimization, phase smoothness, null width control, and radar performance preservation (PSLR & ISLR).
* **Pareto-Frontier Analysis:** Utilized the Optuna framework to systematically explore the hyperparameter space, generating a Pareto Frontier that clearly illustrates the trade-offs between Null Energy, PSLR, and ISLR.
...
* **Receiver Design & Channel Testing:** Architect a custom Maximum-Likelihood receiver, and evaluate the system's dual-functionality performance under an Additive White Gaussian Noise (AWGN) channel using standard metrics (POD for sensing; BER and SER for communication).
